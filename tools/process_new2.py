...
def blank(src):
    ...
    for i in range(np.random.randint(0, 30)):
        ...
        if hole_type == 0:
            ...
            rr, cc = draw.ellipse(c_cent, r_cent, c_rad, r_rad, rotation=np.deg2rad(np.random.randint(0, 360)), shape=(300, 300))
            dst[rr, cc] = 0
        elif hole_type == 2:
            ...
            rr, cc = draw.bezier_curve(r0, c0, r1, c1, r2, c2, weight, (300, 300))
            dst[rr, cc] = 0
        elif hole_type > 2:
